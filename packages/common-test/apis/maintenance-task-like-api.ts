...
import {
  ...
  CryptoStrategies,
  DataOwnerWithType,
  forceUuid, MaintenanceTaskLikeApiImpl,
  ...
  describe('Notification API', function () {
    ...
    beforeAll(async function () {
      ...
      idFilterNotification1 = await ctx.mtApi(hcp1Api).createOrModify(
        ctx.toDSMt(new MaintenanceTask({
    ...
    it('should be able to modify an existing Notification as the creator', async () => {
      ...
      const modifiedNotification = await ctx.mtApi(hcp1Api).createOrModify(
        ctx.toDSMt({ ...ctx.toMtDto(createdNotification), status: 'ongoing' }),
      )
    ...
    it('should be able to filter Notifications by id as the creator', async () => {
      const result = await ctx.mtApi(hcp1Api).filterBy(
        await ctx.newMtFilter(hcp1Api!)
          .forSelf()
    ...
    it('should be able to filter Notifications by id as the delegate', async () => {
      const result = await ctx.mtApi(hcp2Api).filterBy(
        await ctx.newMtFilter(hcp2Api!)
          .forSelf()
          .byIds([idFilterNotification1!.id!, idFilterNotification2!.id!])
          .build()
      )
    ...
    it('should be able to filter Notifications after date', async () => {
      ...
      const result = await ctx.mtApi(hcp1Api).filterBy(
        await ctx.newMtFilter(hcp1Api!).forSelf().afterDate(startTimestamp).build()
      )
      expect(result.rows.length).toBeGreaterThan(0)
    ...
    it('should be able to get all the Notifications as the creator', async () => {
      const result = await ctx.mtApi(hcp1Api).filterBy(
        await ctx.newMtFilter(hcp1Api!).forSelf().build()
      )
      expect(result.rows.length).toBeGreaterThan(0)
    })

    it('should be able to get all the Notifications from multiple paginated lists', async () => {
      const filter = await ctx.newMtFilter(hcp1Api!)
        .forSelf()
        .withType(NotificationTypeEnum.NEW_USER_OWN_DATA_ACCESS)
        .build()
      ...
      const result = await (ctx.mtApi(hcp1Api) as unknown as MaintenanceTaskLikeApiImpl<any>).concatenateFilterResults(
        filter,
        undefined,
