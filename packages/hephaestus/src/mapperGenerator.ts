...
export const mapperGenerator = (project: Project, mapperLocation: string) => {
  const modelClassesWithMapToDecorator = project
    ...
    .filter((classDeclaration) =>
      classDeclaration.getDecorators().some((d) => d.getName() === "mapTo")
    );
  ...
  const mapperFilesToCreate: [ClassDeclaration, string][] =
    modelClassesWithMapToDecorator
      .filter((modelClass) => {
      ...
      .map((modelClass) => {
        return [
          modelClass,
          `${mapperLocation}/${modelClass.getName()}.mapper.ts`,
        ];
      });

  mapperFilesToCreate.map(
    ([modelClass, mapperFilePath]: [ClassDeclaration, string]) => {
      const mapToDecorator = modelClass
        .getDecorators()
        .find((d) => d.getName() === "mapTo")!;
      const mapToDecoratorArg = mapToDecorator
        .getArguments()[0]
        .asKind(SyntaxKind.Identifier)!;
      const iCureTargetClassName = mapToDecoratorArg.getText();
      const className = modelClass.getName()!;
      const isSameName =
        iCureTargetClassName === className + "Entity" ||
        iCureTargetClassName === className + "Dto" ||
        iCureTargetClassName === className;
      ...
      return project.createSourceFile(mapperFilePath, (writer) => {
        writer
          ...
          .conditionalWriteLine(
            !isSameName,
            `import { ${iCureTargetClassName} } from "@icure/api"`
          )
          .conditionalWriteLine(
            isSameName,
            `import { ${className} as ${iCureTargetClassName} } from "@icure/api"`
          )
          .blankLine();
      });
    }
  ...
  const mapperFiles: [string, SourceFile, string, ClassDeclaration?][] =
    getMapperSourceFiles()
      .map(([mapperClassName, mapperSourceFile]) => {
        const modelClass = modelClassesWithMapToDecorator.find(
          (modelClass) => modelClass.getName() === mapperClassName
        );

        return [
          mapperClassName,
          mapperSourceFile,
          `initialize${mapperClassName}Mapper`,
          modelClass,
        ];
      })
      .filter(([, , , modelClass]) => modelClass !== undefined) as [
      string,
      SourceFile,
      string,
      ClassDeclaration
  ...
  mapperFiles.forEach(
    ([mapperClassName, mapperSourceFile, functionName, modelClass]) => {
      const mapToDecorator = modelClass!
        .getDecorators()
        .find((d) => d.getName() === "mapTo")!;
      const mapToDecoratorArg = mapToDecorator
        .getArguments()[0]
        .asKind(SyntaxKind.Identifier)!;
      const iCureTargetClassDeclaration = mapToDecoratorArg
        .getSymbol()!
        .isAlias()
        ? mapToDecoratorArg
            .getSymbol()!
      ...
      const toDtoFunctions = iCureTargetClassDeclaration
        ...
        .map(({ name, type }) => {
          return {
            functionName: `to${iCureTargetClassName}${name
              .slice(0, 1)
      ...
      const toDomainFunctions = modelClass
        ...
        .map(({ name, type }) => {
          return {
            functionName: `to${className}${name
              .slice(0, 1)
              .toUpperCase()}${name.slice(1)}`,
            propertyName: name,
            type,
      ...
      const toDtoFunctionsToCreate = toDtoFunctions?.filter(
        ({ functionName }) =>
          !mapperFileFunctions.some((mff) => mff.getName() === functionName)
      );
      const toDomainFunctionsToCreate = toDomainFunctions?.filter(
        ({ functionName }) =>
          !mapperFileFunctions.some((mff) => mff.getName() === functionName)
      );
