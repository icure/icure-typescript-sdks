...
export function fromJSONGenerator(classDeclaration: ClassDeclaration, bundle: ClassBundle) {
    ...
    const mandatoryComponents = [...bundle.components.entries()].filter(([, component]) => !component.nullable && !component.optional && component.typeName !== 'EntityId')

    classDeclaration.addConstructor({
        parameters: [
            {
                name: 'json',
                type: `Partial<I${classDeclaration.getName()}> ${
                    mandatoryComponents.length
                        ? `& { ${mandatoryComponents.map(([propertyName, component]) => {
