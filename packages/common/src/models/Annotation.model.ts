...
export class Annotation {
    ...
    static fromJSON(pojo: any): Annotation {
        return new Annotation({id: pojo["id"], tags: pojo["tags"]?.map((item: any) => CodingReference.fromJSON(item)), author: pojo["author"], created: pojo["created"], modified: pojo["modified"], markdown: pojo["markdown"] ? new Map(Object.entries(pojo["markdown"]).map(([k, v]: [any, any]) => [k, v])) : undefined, target: pojo["target"], confidential: pojo["confidential"], encryptedSelf: pojo["encryptedSelf"]})
    }
}
