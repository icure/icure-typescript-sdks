...
export class User {
  ...
  "authenticationTokens": Map<string, AuthenticationToken>;
    ...
    static fromJSON(pojo: any): User {
        return new User({id: pojo["id"], rev: pojo["rev"], deletionDate: pojo["deletionDate"], created: pojo["created"], name: pojo["name"], properties: new Set(pojo["properties"].map((item: any) => Property.fromJSON(item))), roles: new Set(pojo["roles"].map((item: any) => item)), login: pojo["login"], passwordHash: pojo["passwordHash"], secret: pojo["secret"], use2fa: pojo["use2fa"], groupId: pojo["groupId"], healthcarePartyId: pojo["healthcarePartyId"], patientId: pojo["patientId"], deviceId: pojo["deviceId"], sharingDataWith: new Map(Object.entries(pojo["sharingDataWith"]).map(([k, v]: [any, any]) => [k, new Set(v.map((item: any) => item))])), email: pojo["email"], mobilePhone: pojo["mobilePhone"], authenticationTokens: new Map(Object.entries(pojo["authenticationTokens"]).map(([k, v]: [any, any]) => [k, AuthenticationToken.fromJSON(v)]))})
    }
}
