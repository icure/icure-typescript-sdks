...
export class SortableFilterBuilderAccumulator<T> {
    ...
    async getAndSortFilters(): Promise<Filter<T>[]> {
        ...
        return sortKey !== -1 ? [listOfCombinedFilters[sortKey].filter, ...listOfCombinedFilters.filter((_, index) => index !== sortKey).map((f) => f.filter)] : listOfCombinedFilters.map((f) => f.filter)
    }
    ...
    private combineByIdsFilters(filters: AwaitedFilter<T>[]) {
        ...
        const aggregation = filters.reduce((aggregated, current) => {
            const newFilter = {
                ...aggregated.filter,
                [aggregated.idKey!]: this.extractIdsFromFilter(aggregated.filter, aggregated.idKey!).filter((x) => this.extractIdsFromFilter(current.filter, aggregated.idKey!).includes(x)),
            } as Filter<T>
            return {
                ...aggregated,
                filter: newFilter,
