...
import {HealthElementByHealthcarePartyFilter} from "../healthelement/HealthcareElementByHealthcarePartyFilter";
...
export class HealthElementFilterWithDataOwner
  extends SortableFilterBuilder<HealthElement, DeviceFilterSortStepDecorator>
  implements BaseHealthElementFilterBuilder<HealthElementFilterWithDataOwner>, FilterBuilder<HealthElement>
{
  ...
  forPatients(patients: Patient[]): HealthElementFilterWithDataOwner {
    const filter = this._dataOwnerId.then((id) => {
      return Promise.all(
        patients.map((p) => this.api.baseApi.cryptoApi.xapi.secretIdsOf({ entity: p!, type: 'Patient' }, undefined))
      )
