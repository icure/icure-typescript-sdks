...
export class Observation {
    ...
    static fromJSON(pojo: any): Observation {
        ...
        else instance.transactionId = pojo["transactionId"]
        if (pojo["identifiers"] === undefined) instance.identifiers = undefined
        else instance.identifiers = pojo["identifiers"]?.map((item: any) => Identifier.fromJSON(item))
        if (pojo["batchId"] === undefined) instance.batchId = undefined
        else instance.batchId = pojo["batchId"]
        if (pojo["healthcareElementIds"] === undefined) instance.healthcareElementIds = undefined
        else instance.healthcareElementIds = pojo["healthcareElementIds"]?.map((item: any) => item)
        if (pojo["index"] === undefined) instance.index = undefined
        else instance.index = pojo["index"]
        if (pojo["component"] === undefined) instance.component = undefined
        else instance.component = !!pojo["component"] ? Component.fromJSON(pojo["component"]) : undefined
        if (pojo["valueDate"] === undefined) instance.valueDate = undefined
        else instance.valueDate = pojo["valueDate"]
        ...
        else instance.performer = pojo["performer"]
        if (pojo["localContent"] === undefined) instance.localContent = undefined
        else instance.localContent = new Map(pojo["localContent"]?.map(([k, v]: [any, any]) => [k, LocalComponent.fromJSON(v)]) ?? [])
        if (pojo["qualifiedLinks"] === undefined) instance.qualifiedLinks = undefined
        else instance.qualifiedLinks = new Map(pojo["qualifiedLinks"]?.map(([k, v]: [any, any]) => [k, new Map(v.map(([k, v]: [any, any]) => [k, v]))]) ?? [])
        if (pojo["codes"] === undefined) instance.codes = undefined
        else instance.codes = pojo["codes"]?.map((item: any) => CodingReference.fromJSON(item))
        ...
        else instance.note = pojo["note"]?.map((item: any) => Annotation.fromJSON(item))
        return instance
    }
}
