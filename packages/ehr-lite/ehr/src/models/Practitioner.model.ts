...
export class Practitioner {
    ...
    static fromJSON(pojo: any): Practitioner {
        return new Practitioner({id: pojo["id"], rev: pojo["rev"], created: pojo["created"], modified: pojo["modified"], identifiers: pojo["identifiers"]?.map((item: any) => Identifier.fromJSON(item)), tags: pojo["tags"]?.map((item: any) => CodingReference.fromJSON(item)), codes: pojo["codes"]?.map((item: any) => CodingReference.fromJSON(item)), deletionDate: pojo["deletionDate"], name: pojo["name"], lastName: pojo["lastName"], firstName: pojo["firstName"], names: pojo["names"]?.map((item: any) => HumanName.fromJSON(item)), gender: pojo["gender"], civility: pojo["civility"], speciality: pojo["speciality"], parentId: pojo["parentId"], userId: pojo["userId"], addresses: pojo["addresses"]?.map((item: any) => Location.fromJSON(item)), languages: pojo["languages"]?.map((item: any) => item), picture: !!pojo["picture"] ? b64_2ab(pojo["picture"]) : undefined, specialityCodes: pojo["specialityCodes"]?.map((item: any) => CodingReference.fromJSON(item)), description: pojo["description"], properties: pojo["properties"]?.map((item: any) => Property.fromJSON(item)), systemMetaData: !!pojo["systemMetaData"] ? SystemMetaDataOwner.fromJSON(pojo["systemMetaData"]) : undefined})
    }
}
