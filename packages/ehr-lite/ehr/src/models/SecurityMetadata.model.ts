...
export class SecurityMetadata {
    ...
    static fromJSON(pojo: any): SecurityMetadata {
        return new SecurityMetadata({
            secureDelegations: pojo['secureDelegations'] ? new Map(Object.entries(pojo['secureDelegations']).map(([k, v]: [any, any]) => [k, SecureDelegation.fromJSON(v)])) : undefined,
            keysEquivalences: pojo['keysEquivalences'] ? new Map(Object.entries(pojo['keysEquivalences']).map(([k, v]: [any, any]) => [k, v])) : undefined,
        })
    }
}
