...
function forMember_HealthElement_id() {
    return forMember<Condition, HealthElement>(v => v.id, mapFrom(v => v.id))
}

function forMember_HealthElement_identifiers() {
    return forMember<Condition, HealthElement>(v => v.identifiers, mapWith(Identifier, IdentifierEntity, v => v.identifiers))
}

function forMember_HealthElement_rev() {
    return forMember<Condition, HealthElement>(v => v.rev, mapFrom(v => v.rev))
}

function forMember_HealthElement_created() {
    return forMember<Condition, HealthElement>(v => v.created, mapFrom(v => v.created))
}

function forMember_HealthElement_modified() {
    return forMember<Condition, HealthElement>(v => v.modified, mapFrom(v => v.modified))
}

function forMember_HealthElement_author() {
    return forMember<Condition, HealthElement>(v => v.author, mapFrom(v => v.author))
}

function forMember_HealthElement_responsible() {
    return forMember<Condition, HealthElement>(v => v.responsible, mapFrom(v => v.responsible))
}

function forMember_HealthElement_medicalLocationId() {
    return forMember<Condition, HealthElement>(v => v.medicalLocationId, mapFrom(v => v.medicalLocationId))
}
...
function forMember_HealthElement_codes() {
    return forMember<Condition, HealthElement>(v => v.codes, mapWith(CodingReference, CodeStub, v => !!v.codes ? [...v.codes] : undefined))
}

function forMember_HealthElement_endOfLife() {
    return forMember<Condition, HealthElement>(v => v.endOfLife, mapFrom(v => v.endOfLife))
}

function forMember_HealthElement_deletionDate() {
    return forMember<Condition, HealthElement>(v => v.deletionDate, mapFrom(v => v.deletionDate))
}

function forMember_HealthElement_healthElementId() {
    return forMember<Condition, HealthElement>(v => v.healthElementId, mapFrom(v => v.healthcareElementId))
}

function forMember_HealthElement_valueDate() {
    return forMember<Condition, HealthElement>(v => v.valueDate, mapFrom(v => v.recordedDate))
}

function forMember_HealthElement_openingDate() {
    return forMember<Condition, HealthElement>(v => v.openingDate, mapFrom(v => v.openingDate))
}

function forMember_HealthElement_closingDate() {
    return forMember<Condition, HealthElement>(v => v.closingDate, mapFrom(v => v.closingDate))
}

function forMember_HealthElement_descr() {
    return forMember<Condition, HealthElement>(v => v.descr, mapFrom(v => v.description))
}
...
function forMember_HealthElement_notes() {
    return forMember<Condition, HealthElement>(v => v.notes, mapWith(Annotation, AnnotationEntity, v => v.notes))
}
...
function forMember_HealthElement_secretForeignKeys() {
    return forMember<Condition, HealthElement>(v => v.secretForeignKeys, mapFrom(v => exportSecretForeignKeys(v.systemMetaData)))
}

function forMember_HealthElement_cryptedForeignKeys() {
    return forMember<Condition, HealthElement>(v => v.cryptedForeignKeys, mapFrom(v => {
        const cryptedForeignKeys = exportCryptedForeignKeys(v.systemMetaData)
...
function forMember_HealthElement_delegations() {
    return forMember<Condition, HealthElement>(v => v.delegations, mapFrom(v => {
        const delegations = exportDelegations(v.systemMetaData)
...
function forMember_HealthElement_encryptionKeys() {
    return forMember<Condition, HealthElement>(v => v.encryptionKeys, mapFrom(v => {
        const encryptionKeys = exportEncryptionKeys(v.systemMetaData)
...
function forMember_HealthElement_encryptedSelf() {
    return forMember<Condition, HealthElement>(v => v.encryptedSelf, mapFrom(v => exportEncryptedSelf(v.systemMetaData)))
}

function forMember_HealthElement_securityMetadata() {
    return forMember<Condition, HealthElement>(v => v.securityMetadata, mapWith(SecurityMetadata, SecurityMetadataEntity, v => exportSecurityMetadata(v.systemMetaData)))
}
...
function forMember_Condition_id() {
    return forMember<HealthElement, Condition>(v => v.id, mapFrom(v => v.id))
}

function forMember_Condition_identifiers() {
    return forMember<HealthElement, Condition>(v => v.identifiers, mapWith(Identifier, IdentifierEntity, v => v.identifiers))
}

function forMember_Condition_rev() {
    return forMember<HealthElement, Condition>(v => v.rev, mapFrom(v => v.rev))
}

function forMember_Condition_created() {
    return forMember<HealthElement, Condition>(v => v.created, mapFrom(v => v.created))
}

function forMember_Condition_modified() {
    return forMember<HealthElement, Condition>(v => v.modified, mapFrom(v => v.modified))
}

function forMember_Condition_author() {
    return forMember<HealthElement, Condition>(v => v.author, mapFrom(v => v.author))
}

function forMember_Condition_responsible() {
    return forMember<HealthElement, Condition>(v => v.responsible, mapFrom(v => v.responsible))
}

function forMember_Condition_medicalLocationId() {
    return forMember<HealthElement, Condition>(v => v.medicalLocationId, mapFrom(v => v.medicalLocationId))
}

function forMember_Condition_clinicalStatus() {
    return forMember<HealthElement, Condition>(v => v.clinicalStatus, mapFrom(v => v.tags?.find(v => v.context === 'clinicalStatus')?.type))
}

function forMember_Condition_verificationStatus() {
    return forMember<HealthElement, Condition>(v => v.verificationStatus, mapFrom(v => v.tags?.find(v => v.context === 'verificationStatus')?.type))
}

function forMember_Condition_category() {
    return forMember<HealthElement, Condition>(v => v.category, mapFrom(v => v.tags?.find(v => v.context === 'category')?.type))
}

function forMember_Condition_severity() {
    return forMember<HealthElement, Condition>(v => v.severity, mapFrom(v => v.tags?.find(v => v.context === 'severity')?.type))
}

function forMember_Condition_bodySite() {
    return forMember<HealthElement, Condition>(v => v.bodySite, mapWith(CodingReference, CodeStub, v => v.tags?.filter(v => v.context === 'bodySite')))
}

function forMember_Condition_tags() {
    return forMember<HealthElement, Condition>(v => v.tags, mapWith(CodingReference, CodeStub, v =>  v.tags?.filter(v => !!v.context ? ['clinicalStatus', 'verificationStatus', 'category', 'severity', 'bodySite'].includes(v.context) : undefined)))
}

function forMember_Condition_codes() {
    return forMember<HealthElement, Condition>(v => v.codes, mapWith(CodingReference, CodeStub, v => v.codes))
}

function forMember_Condition_endOfLife() {
    return forMember<HealthElement, Condition>(v => v.endOfLife, mapFrom(v => v.endOfLife))
}

function forMember_Condition_deletionDate() {
    return forMember<HealthElement, Condition>(v => v.deletionDate, mapFrom(v => v.deletionDate))
}

function forMember_Condition_healthcareElementId() {
    return forMember<HealthElement, Condition>(v => v.healthcareElementId, mapFrom(v => v.healthElementId))
}

function forMember_Condition_recordedDate() {
    return forMember<HealthElement, Condition>(v => v.recordedDate, mapFrom(v => v.valueDate))
}

function forMember_Condition_openingDate() {
    return forMember<HealthElement, Condition>(v => v.openingDate, mapFrom(v => v.openingDate))
}

function forMember_Condition_closingDate() {
    return forMember<HealthElement, Condition>(v => v.closingDate, mapFrom(v => v.closingDate))
}

function forMember_Condition_description() {
    return forMember<HealthElement, Condition>(v => v.description, mapFrom(v => v.descr))
}

function forMember_Condition_notes() {
    return forMember<HealthElement, Condition>(v => v.notes, mapWith(Annotation, AnnotationEntity, v => v.notes))
}

function forMember_Condition_systemMetaData() {
    return forMember<HealthElement, Condition>(v => v.systemMetaData, mapFrom(v => new SystemMetaDataEncrypted({
        ...
        encryptionKeys: !!v.encryptionKeys ? new Map(Object.entries(v.encryptionKeys).map(([k, v]) => [k, mapper.mapArray(v, Delegation, DelegationEntity)])) : undefined,
