import {
  AuthenticationApi,
  CryptoStrategies, DataOwnerWithType,
  ErrorHandler,
  ErrorHandlerImpl, formatICureApiUrl, ICURE_CLOUD_URL, MSG_GW_CLOUD_URL,
  Sanitizer,
  SanitizerImpl
...
export class AnonymousMedTechApiBuilder {
  ...
  withCryptoStrategies(strategies: CryptoStrategies<DataOwnerWithType>): AnonymousMedTechApiBuilder {
  ...
  async build(): Promise<AnonymousEHRLiteApi> {
    ...
    return new AnonymousEHRLiteApi(
      iCureBaseUrl,
      msgGwUrl,
      msgGwSpecId,
      authProcessByEmailId,
