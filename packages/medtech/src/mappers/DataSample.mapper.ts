...
function toServiceQualifiedLinks(domain: DataSample): { [key: string]: { [key: string]: string } } | undefined {
  return domain.qualifiedLinks
    ? Object.fromEntries([...domain.qualifiedLinks.entries()].map(([key, value]) => [key, Object.fromEntries([...value.entries()])]))
    : undefined
}
...
function toDataSampleQualifiedLinks(dto: Service): Map<string, Map<string, string>> {
  return dto.qualifiedLinks
    ? new Map([...Object.entries(dto.qualifiedLinks)].map(([key, value]) => [key, new Map([...Object.entries(value)])]))
    : new Map()
}
