...
const moves: Array<(s: string) => string> = [(s) => s.replace(/classes\/(.*Api.md)/, 'entrypoints/$1'), (s) => s.replace(/classes\/(.*Builder.md)/, 'builders/$1'), (s) => s.replace(/classes\/(.*Filter.md)/, 'filters/$1'), (s) => s.replace(/interfaces\/(.*Api.md)/, 'apis/$1')]

function fixLinks(line: string): string {
    return moves.reduce((s, f) => s.replace(/\((.+?\.md)([)#])/g, (match, link, end) => `(${f(link)}${end}`), line).replace(/\((.+?)\.md([)#])/g, '($1$2')
}
