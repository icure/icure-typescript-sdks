...
describe('User API', () => {
  ...
  it('If sharedDataType already shared with ownerIds : 200 ok, return user (no treatment needed)', async () => {
    ...
    assert(userUpdatedWithUpdatedDelegationsOnMedicalInformation.sharingDataWith.get("medicalInformation")?.has(delegation))
    ...
    assert(userUpdatedWithUpdatedDelegationsOnMedicalInformationButNoChanges.sharingDataWith.get("medicalInformation")?.has(delegation))
    assert(
  ...
  it('A user should be able to share data with another dataOwner, and stop sharing data with him later', async () => {
    ...
    assert(userUpdatedWithUpdatedDelegationsOnAll.sharingDataWith.get("all")?.has(delegation))
    assert(!userUpdatedWithUpdatedDelegationsOnAll.sharingDataWith?.get("medicalInformation")?.has(delegation))
    ...
    assert(userUpdatedWithRemovedDelegationsOnMedicalInformation.sharingDataWith.get("all")?.has(delegation))
    assert(!userUpdatedWithRemovedDelegationsOnMedicalInformation.sharingDataWith?.get("medicalInformation")?.has(delegation))
  }).timeout(60000)
})
