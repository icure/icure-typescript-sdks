...
describe('Authentication API', () => {
  ...
  it("Impossible to use authenticationApi if msgGtwUrl, msgGtwSpecId and authProcessId haven't been provided", async () => {
    ...
    // Given
    let api = await new MedTechApi.Builder()
  ...
  it('Patient should be able to retrieve its keys when re-login', async () => {
    ...
    const newApi = await new MedTechApi.Builder()
  ...
  it('A patient may login with a new RSA keypair and access his previous data if he gave access to its new key with his previous private key', async () => {
    ...
    const keysAfterLoss = loginAuthResult.api.dataOwnerApi.getPublicKeysOf(
      await loginAuthResult.api.dataOwnerApi.getDataOwner(patApiAndUser.user.patientId!)
    )
    ...
    expect(await TestUtils.createDataSampleForPatient(loginAuthResult.api, currentPatient)).to.not.be.undefined

    // But can't access (v8) / decrypt (v7) previous ones
    const retrieved = await loginAuthResult.api.dataSampleApi.get(createdDataSample.id!)
    ...
    // Then
    await loginAuthResult.api.forceReload()
    const previousDataSample = await loginAuthResult.api.dataSampleApi.getDataSample(createdDataSample.id!)
  ...
  it('A patient may login with a new RSA keypair and access his previous data only when a delegate gave him access back', async () => {
    ...
    expect(await TestUtils.createDataSampleForPatient(loginAuthResult.api, currentPatient)).to.not.be.undefined
    // But can't access (v8) / decrypt (v7) previous ones
    const retrieved = await loginAuthResult.api.dataSampleApi.getDataSample(createdDataSample.id!)
    expect(retrieved).to.not.be.undefined
    expect(Object.keys(retrieved.content ?? {})).to.have.length(0, 'Content should not be decrypted (emtpy obj or undefined)')
