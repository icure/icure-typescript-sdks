...
describe('Healthcare Element Filters Test', function () {
    ...
    it('Can filter Healthcare Elements by union filter', async function () {
        const tagFilter = await new HealthcareElementFilter(hcp1Api).forDataOwner(hcp1User.healthcarePartyId!).byLabelCodeFilter('SNOMEDCT', '617').build()
        const codeFilter = await new HealthcareElementFilter(hcp1Api).forDataOwner(hcp1User.healthcarePartyId!).byLabelCodeFilter(undefined, undefined, 'SNOMEDCT', '617').build()
        ...
        elements.rows.forEach((he) => {
            expect(he).to.satisfy((e: HealthcareElement) => {
                return Array.from(e.codes).some((it) => it.type === 'SNOMEDCT' && it.code === '617') || Array.from(e.labels).some((it) => it.type === 'SNOMEDCT' && it.code === '617')
            })
        })
    })
    ...
    it('Intersection between disjoint sets return empty result', async function () {
        const tagFilter = await new HealthcareElementFilter(hcp1Api).forDataOwner(hcp1User.healthcarePartyId!).byLabelCodeFilter('SNOMEDCT', '617').build()
        const labelCodeFilter = await new HealthcareElementFilter(hcp1Api).forDataOwner(hcp1User.healthcarePartyId!).byLabelCodeFilter(undefined, undefined, 'SNOMEDCT', '617').build()
