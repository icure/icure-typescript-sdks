...
const firstDelegationWithNullTags = () =>
  new Delegation({
    owner: 'owner',
    delegatedTo: 'delegate',
    key: 'key',
...
describe('Equality functions test', () => {
  ...
  it('deepEquality should be able to check if two sets are equal independently from the order of the elements', () => {
    assert(deepEquality(firstSet(), new Set<DummyData>([new DummyData('c', 'd'), new DummyData('a', 'b')])))
  })

  it('deepEquality should fail if the Sets have different lengths', () => {
    assert(!deepEquality(firstSet(), new Set<DummyData>([new DummyData('a', 'b')])))
  })

  it('deepEquality should fail if the Sets have different elements', () => {
    assert(!deepEquality(firstSet(), new Set<DummyData>([new DummyData('a', 'b'), new DummyData('a', 'b')])))
  })
  ...
  it('deepEquality should be able to check if two Delegations are equal if the tags are in different order', () => {
    assert(deepEquality(new Delegation({ tags: ['b', 'a'] }), new Delegation({ tags: ['a', 'b'] })))
  })
  ...
  it('deepEquality should fail if tags have different lengths', () => {
    assert(!deepEquality(new Delegation({ tags: ['b', 'a', 'c'] }), new Delegation({ tags: ['a', 'b'] })))
  })

  it('deepEquality should fail if tags have different elements', () => {
    assert(!deepEquality(new Delegation({ tags: ['b', 'c'] }), new Delegation({ tags: ['a', 'b'] })))
  })

  it('deepEquality should fail if owner is different', () => {
    assert(!deepEquality(new Delegation({ owner: 'owner' }), new Delegation({ owner: 'owner1' })))
  })

  it('deepEquality should fail if delegatedTo is different', () => {
    assert(!deepEquality(new Delegation({ delegatedTo: 'delegate' }), new Delegation({ delegatedTo: 'delegate1' })))
  })

  it('deepEquality should fail if key is different', () => {
    assert(!deepEquality(new Delegation({ key: 'key' }), new Delegation({ key: 'key2' })))
  })
  ...
  it('deepEquality should be able to check if two systemMetaDataEncrypted are equal if secretForeignKeys are in different order', () => {
    assert(
      deepEquality(new SystemMetaDataEncrypted({ secretForeignKeys: ['s2', 's1'] }), new SystemMetaDataEncrypted({ secretForeignKeys: ['s1', 's2'] }))
    )
  })
